function [X,U,S,J,X_hat_OL,X_bar_OL,V_OL,S_OL,J_wc,time,Theta_HC_t]=RALMPC_Convex_Online(x0,SS,Q_func,A_0,A_1,A_2,B_0,B_1,B_2,A_star,B_star,H_w,h_w,W_V,Q,R,K,F,G,d_bar,L_B,c,H,B_p,n,m,N,p,Theta_HC0,theta_bar0,eta_0,rho_theta0,Delta,mu,Ts,numberitertions,disturbance_deter)
%import Casadi
import casadi.*
...
for h=1:numberitertions
    ...
    while t<20
        ...
        elseif t>0 
            y_OL=Y_OL{end};
            y_init=[y_OL(1:n*N,1);zeros(n,1);y_OL(n*(N+1)+1:2*n*(N+1)-n,1);zeros(n,1);y_OL(2*n*(N+1)+1:2*n*(N+1)+m*(N-1),1);zeros(m,1);y_OL(2*n*(N+1)+m*N+1:2*n*(N+1)+m*N+N,1);0;y_OL(2*n*(N+1)+m*N+N+2);y_OL(2*n*(N+1)+m*N+N+1+1+1:end)];
        else
           for k=0:N-1
                y_init=[y_init;(1-(rho_theta0+eta_0*L_B)^k)/(1-(rho_theta0+eta_0*L_B))*d_bar];
            end 
        end
        y_init=[y_init;zeros(n*(N+1)+n*(N+1)+m*N+(N+1)+1+L-size(y_init,1),1)];
        y_init=y_init(1:n*(N+1)+n*(N+1)+m*N+(N+1)+1+L);
        %Create optimization problem for casadi 
        y=MX.sym('y',n*(N+1)+n*(N+1)+m*N+(N+1)+1+L);%Create the optimization variable
        ...
        lby=[-inf*ones(n*(N+1)+n*(N+1)+m*N+(N+1)+1,1);zeros(L,1)];
        uby=[inf*ones(n*(N+1)+n*(N+1)+m*N+(N+1)+1,1);ones(L,1)];
    ...
    [SS,Q_func,L]=get_updateSampleSet(SS,Q_func,X_bar_OL{h},J_wc{h},V_OL{h},S_OL{h});
    toc
end
end
