...
for h=1:numberitertions
    ...
    while t<60
        ...
        elseif t>0 && h==1
            y_OL=Y_OL{end};
            y_init=[y_OL(n+1:n*(N+1),1);zeros(n,1);y_OL(n*(N+1)+n+1:2*n*(N+1),1);zeros(n,1);y_OL(2*n*(N+1)+m+1:2*n*(N+1)+m*N,1);zeros(m,1);y_OL(2*n*(N+1)+m*N+1:2*n*(N+1)+m*N+N,1);0;y_OL(2*n*(N+1)+m*N+N+2);y_OL(2*n*(N+1)+m*N+N+1+1+1:end)];
        else
           for k=0:N-1
                y_init=[y_init;(1-(rho_theta0+eta_0*L_B)^k)/(1-(rho_theta0+eta_0*L_B))*d_bar];
            end 
